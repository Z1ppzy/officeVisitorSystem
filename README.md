...
phpMyAdmin будет доступен на http://localhost:8080.

При старте контейнер применяет миграции и заполняет демо-данные: сотрудников, посетителей, визиты и аккаунты пользователей.

## phpMyAdmin

После запуска Docker Compose можно открыть http://localhost:8080 и работать с базой `visitors_db`.

Данные для входа:

| Пользователь | Пароль |
| --- | --- |
| `visitor_user` | `visitor_password` |
